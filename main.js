...
ipcMain.on('openDeleteEventDialog', function(event, arg) {
	...
	getDeleteEventDialog.once('ready-to-show', function() {
		...
		getDeleteEventDialog.send('getDeleteEventEventList', {
			item: item,
			events: allEvents.map(function(a) {
				return a['unique_event_name'];
			})
		});
	});
						...
						for (var j = 0; j < sortedKeys.length; j++) {
							...
							if (name == d['field_name'] &&
								...
								if (part.length > 1 && part[0].toLowerCase().indexOf('(') == -1) { // instead of removing "(" we should test the type of the variable
									var ch = d['select_choices_or_calculations'].split("|").map(function(a) {
...
function filterDataDictionary(token) {
	...
	request({
		...
		data = body;
		writeLog("number of instruments before filtering: " + Object.keys(instruments).length);
